...
describe("LoginPage", () => {
  ...
  it("renders the login form", () => {
    ...
    expect(screen.getByRole("heading", { name: "Daxil olun" })).toBeInTheDocument();
    expect(screen.getByLabelText("İstifadəçi adı, e-poçt və ya telefon")).toBeInTheDocument();
  ...
  it("shows validation messages before submit", async () => {
    ...
    expect(screen.getByText("İstifadəçi adı, e-poçt və ya telefonu daxil edin.")).toBeInTheDocument();
    expect(screen.getByText("Şifrəni daxil edin.")).toBeInTheDocument();
    expect(loginMock).not.toHaveBeenCalled();
  });

  it("redirects admins to the workspace chooser when there is no intended route", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/login"]}>
        <Routes>
          ...
          <Route path="/choose-workspace" element={<div>Workspace destination</div>} />
          <Route path="/account" element={<div>Account destination</div>} />
        </Routes>
      </MemoryRouter>
    );

    await userEvent.type(screen.getByLabelText("İstifadəçi adı, e-poçt və ya telefon"), "admin@mizan.az");
  ...
  it("returns users to the intended route after login", async () => {
    ...
    await userEvent.type(screen.getByLabelText("İstifadəçi adı, e-poçt və ya telefon"), "user@mizan.az");
    await userEvent.type(screen.getByLabelText("Şifrə"), "password123");
    await userEvent.click(screen.getByRole("button", { name: /daxil ol/i }));
